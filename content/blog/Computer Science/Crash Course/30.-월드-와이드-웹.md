...
- 하이퍼링크가 포함된 텍스트는 하이퍼텍스트라 하며, 이를 이용해 문서를 작성할 수 있다.
- 이렇게 작성된 문서를 웹 페이지라고 하며, 각 페이지의 고유한 주소는 URL이라고 한다.
- URL로는 DNS 조회를 처리하며, 이후, 전달받은 IP 주소로 서버 컴퓨터와 통신하게 된다.
- 이렇게, 하이퍼텍스트 웹 페이지를 가져올 때는, HTTP라는 프로토콜을 사용해야 한다.
- 웹 브라우저가 HTTP의 GET 요청을 하면, 아스키 문자 형태의 명령이 서버로 전달된다.
- 서버는 이러한 요청을 받은 후, 해당 요청에 대한 하이퍼텍스트 문서나 정보를 응답한다.
- 하이퍼텍스트 문서는 평범한 문자 형태로 저장/전송되어서, 하이퍼링크를 구분할 수 없다.
- 이를 해결하기 위해, 하이퍼텍스트 문서에 표식을 남기는 언어인 HTML이 개발되었다.
- HTML에서는 표식 열기/닫기를 통해 영역을 구문하며, 표식마다 속성을 지정할 수 있다.
