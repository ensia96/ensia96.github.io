...
<details><summary>오늘날의 검색 엔진과 비슷한 방식으로 작동하는 최초의 웹 검색 엔진은 'JumpStation' 이다.</summary>
...
<details><summary>첫 번째는, 웹에서 발견되는 모든 링크를 따라가는 소프트웨어인 '웹 크롤러(web crawler)' 다.</summary>
...
<details><summary>두 번째는, 크롤러가 방문한 페이지에 등장한 모든 용어(term) 를 기록하는 '색인(index)' 이다.</summary>
...
<details><summary>세 번째는, 수집된 색인들을 참조(consult) 하는 '검색 알고리즘(search algorithm)' 이다.</summary>
...
구글이 유명해진 이유의 상당 부분은, 이러한 문제를 피해 가는 영리한 알고리즘 때문이었다.
...
<details><summary>따라서, 이러한 '백링크(backlink)' 의 수는 보통, 해당 페이지의 품질을 나타내는 지표였다.</summary>

- 특히, 평판이 좋은 사이트에서 연결되는 백링크의 수는, 품질을 인증하는 좋은 신호였다.
...
ISP 는 경쟁사들에게 강력한 인센티브를 요구하며, 타사의 컨텐츠 전송에 제약을 줄 수도 있다.
...
> 고객으로서는, 자신이 선호하는 사이트를 제한하는 ISP 를 사용하고 싶지 않을 것이기 때문이다. 
