...
- 이 때, 'relays + 1' 의 결과는 '1 + 1 = 2' 가 된다.
- 따라서, 위에서 얻은 결과 값 2가 'relays' 에 덮어 씌워진다.
...
지금까지의 예제들을 이해했다면, 함수의 능력과 현대 프로그래밍의 본질에 대해서도 이해한 것이라 할 수 있다.
...
프로그램을 함수로 모듈화했을 때의 효과는 아래와 같다.

- 단 한 명의 프로그래머라도 전체 프로그램을 작성할 수 있게 된다.
...
현대 프로그래밍 언어에는 미리 작성된 함수들의 거대한 집합들이 있기 때문에,  
실제 프로그래머들은 'exponent' 와 같은 것을 작성하는데 시간을 낭비하지 않는다.  
`(ㅗㅜㅑ;; 팩트폭력 멈춰ㅓㅓ!)`

그런 함수들의 집합을 **'라이브러리(Library)'** 라고 부른다.
...
하지만 그 전에, 우리는 **'알고리즘(Algorithm)'** 에 대해 배울 필요가 있다.
