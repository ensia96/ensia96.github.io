...
'함수를 호출하는 함수를 호출하는 함수' 구성을 보고, 추상화의 중요성을 다시 한번 느꼈다.  
`(구성 자체는 번거로워 보이지만, 추상화 계층이 잘 나뉘면 괜찮을지도..?)`

'모듈화로 작업 단위가 나뉘면, 업무 분배/관리는 어떻게 할까?' 하는 생각이 들었다.  
`(작업을 분배하고 관리하는 사람은 머리가 많이 아프지 않을까??)`

'코드 조각을 포장(package)한 함수들의 집합이 라이브러리(library) 이다' 라는 표현이 인상깊었다.  
`(앞으로 패키지와 라이브러리의 차이점은 절대 안 헷갈릴듯 ㅎ)`

`(+ 다음 수업의 주제가 알고리즘이라는 사실에 가슴이 웅장해졌다..)`

(해당 글의 작성 과정은 
<a href='https://github.com/ensia96/ensia96.github.io/pull/102' target='-blank'>post/crash-course/12 (#102)</a>
에서 확인하실 수 있습니다.)
