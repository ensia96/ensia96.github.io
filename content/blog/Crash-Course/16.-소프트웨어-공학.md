...
또, 최신화될 예정인 코드나, 누군가 현재 변경 중인 코드를 건드리지 않으면,  
변경 내용의 충돌(conflict) 이나 같은 작업이 중복되는 것을 방지할 수 있다.

- 수백 명의 프로그래머가 동시에 문제없이 작업할 수 있다.
- 이런 과정을 반복해, 큰 규모의 프로그램을 구성할 수 있다.

<br>

이 때, 버그가 있는 코드를 작성해 커밋하는 것은 문제가 되니, 주의해야 한다.
...
따라서, 서버에 저장된 마스터(master) 버전은 항상 문제없는 상태를 유지해야 한다.

> 오류 없이 컴파일되어, 버그가 거의 없는 상태로 실행되는 상태

<br>

버그가 발생하더라도, 소스 관리 소프트웨어를 이용해 문제를 해결할 수 있다.
...
- 이들은 거대한 소프트웨어를 구성하는 데 도움을 주는 요소들이었다.
