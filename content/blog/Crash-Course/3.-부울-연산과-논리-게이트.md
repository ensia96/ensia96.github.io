...
이렇게 출력선으로 충분한 전류가 흐르지 않게 되어, 결국 'Off' 상태가 된다.
...
<details><summary>출력선에 충분한 전류가 흐르게 되어 출력은 'On' 이 된다.</summary>
...
위 회로를 부정 연산을 처리하는 트랜지스터 회로라고 할 수 있다.

이 회로는 'NOT Gate'(부정 연산 회로) 라고 부른다.
...
앞부분은 참이지만 뒷부분이 거짓이기 때문에,  
전체 명제는 '참 **그리고** 거짓' 의 결과인 거짓이 된다.

<br>

순서를 뒤집더라도 거짓임에는 변함이 없고,  
두 내용이 모두 거짓인 경우에도 전체 명제는 거짓이 된다.

**이 상황을 논리표로 정리하면 다음과 같다.**
...
<details><summary>2. 다음 위치의 트랜지스터에서 전류가 끊겨, 출력은 'Off' 상태가 된다.</summary>
...
<details><summary>반대로, '입력2' 를 담당하는 트랜지스터만 켜도 똑같이 전류가 통하지 않게 되고,</summary>

![](./images/transistor/input1-off-input2-on(and).png)
</details>

<details><summary>모든 입력이 'On' 상태일 경우에만 출력선에 전기가 흐르게 된다.</summary>
...
OR 게이트(논리합 연산 회로)를 만들기 위해서는 약간의 추가 배선이 필요한데,  
두 개의 트랜지스터를 전류원에 각각 연결한 후에 아래와 같이 병렬로 배치하면 된다.
...
<details><summary>이 회로에서 모든 입력이 'Off' 상태인 경우, 출력선까지 전류가 도달하지 못하게 된다.</summary>
...
<details><summary>2. 출력선으로 전류가 흐르게 되어, 출력은 'On' 상태가 된다.</summary>
...
<details><summary>반대로, ‘입력2’ 를 담당하는 트랜지스터만 켜도 똑같이 전류가 통하게 되고,</summary>

![](./images/transistor/input1-off-input2-on(or).png)
</details>

<details><summary>모든 입력이 'On' 상태인 경우에도 출력선에 전기가 흐르게 된다.</summary>
...
논리 게이트를 활용하여 새로운 회로를 구성하는 경우,

트랜지스터, 회로 연결 등 세부 요소가 많아질수록  
전체 구성에 대해 집중하기 어려울 것이다.

하지만 이 때, 기호를 활용하면 난잡했던 회로도는 단순해질 것이다.

>
이렇게 단순화하여 생각하는 방식인 추상화를 활용하면,  
**전체적인 복잡도는 유지하면서 더 중요한 부분에 신경쓰는 것이 수월해진다.**

이런 추상화의 예로, 'Exclusive OR' 이라는 논리 연산이 있는데,  
이 논리 연산에 대해 알아보며 추상화의 유용함을 경험해보자.  

> 'Exclusive OR' 은 매우 유용한 논리 연산이며, 줄여서 'XOR' 이라고도 한다.

# 7. 배타적 논리합 연산(XOR)

배타적 논리합 연산은 일반적인 논리합 연산과 비슷하지만,  
'두 입력이 모두 참인 경우, 출력은 거짓이 된다' 라는 차이점이 있다.
...
트랜지스터를 이용해 회로를 구성하는 것이 조금은 복잡하겠지만,  
위에서 살펴본 3가지 기본 연산 회로들을 활용하여 구성해보자.
...
이 때, 논리합 연산과 다르게 동작하기 때문에, 추가적인 논리 회로가 필요하다.

<details><summary>논리곱 회로를 이렇게 추가하는 경우,</summary>

![](./images/transistor/or+and(xor).png)
</details>

<details><summary>입력이 모두 참일 때 여전히 출력은 참이다.</summary>

![](./images/transistor/or+and-result(xor).png)
</details>

<details><summary>하지만, 여기서 바로 뒤에 부정 연산 회로를 추가하게 되면,</summary>

- 논리곱 회로(AND)의 결과가 부정되어 거짓이 된다.
![](./images/transistor/or+and+not(xor).png)
</details>

<details><summary>현재 회로의 마지막 부분에 논리곱 회로를 추가하면,</summary>

- 부정된 논리곱의 결과(AND-NOT)와 논리합(OR)의 결과를 입력받아 연산하게 되고,  
  최종적으로 참과 거짓의 논리곱(AND) 결과인 거짓이 출력된다.
...
이렇게 두 입력이 모두 참인 경우,  
배타적 논리합 연산과 결과가 같은 것을 알 수 있다.

<details><summary>나머지 경우도 연산 결과가 표와 일치하기 때문에,</summary>
