...
![](./images/memory/memory-module.png)
...
![](./images/memory/Random-Access-Memory.png)
...
기억 장치라는 구성 요소가 어떻게 동작하는 지에 대해 더 쉽게 이해할 수 있었다.
...
출력을 다시 입력으로 되돌려보내는 루프백이라는 개념을 배웠다.

단편적인 값을 기억하는 회로를 조합해 래치를 만드는 방식이 신기했다.

set과 reset을 조합하는 복잡한 과정을 정보와 쓰기 가능 입력으로 단순화한 것을 보고,  
컴퓨팅에 논리적인 사고력을 적용하는 방식에 대해 다시 생각해보게 되었다.
...
- 여러 입력 신호를 받아, 그 중 하나를 선택하여 해당하는 회로에 전달하는 멀티플렉서

## 3.

- 여러 개의 기억 장치와 회로들이 결합되어 구성된 하나의 단위 요소인 메모리 모듈
