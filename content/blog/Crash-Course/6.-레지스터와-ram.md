...
<details><summary>이 때, 행을 담당하는 멀티플렉서는 회로를 기준으로 세로로 배치된다.</summary>

![](./images/memory/2-multiplexer-row.png)
</details>

<details><summary>반대로, 열을 담당하는 멀티플렉서는 회로를 기준으로 가로로 배치된다.</summary>
...
하지만, 256비트만으로는 할 수 있는 것이 거의 없기 때문에 확장이 필요하다.

## 7-1. 구성

256비트 기억 장치의 확장 과정을 그림과 함께 살펴보자.

<details><summary>1. 8개의 256비트 메모리를 레지스터를 구성했던 방식으로 나란히 놓는다.</summary>

![](./images/memory/8-256bit-memory.png)
</details>

<details><summary>2. 모든 메모리의 주소, 정보 회로는 모으고, 쓰기 가능, 읽기 가능 회로는 합친다.</summary>
...
이렇게, 일렬로 연결된 메모리 모듈과 회로를 하나의 요소로 단순화해봤다.

- 주소를 지정할 수 있는 기억 장치의 뱅크(bank)로 취급될 수 있다.
...
`검색해보면 확실하게 '이거다!' 할 만한 내용이 없고, 크게 두 가지 유형으로 설명된다.`

1. 기억 장치와 관련된 용어 중, 'bank'(뱅크 혹은 메모리 뱅크) 가 표현하는 것은  
   프로세서가 한 번에 처리할 수 있는 최소 메모리 양이다.
2. 프로세서에 데이터가 지속적으로 흘러갈 수 있도록 순차적으로 작동하는 장치이며,  
