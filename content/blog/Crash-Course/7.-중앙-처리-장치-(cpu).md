...
`(최초의 프로세서임에도 불구하고 초당 74만 개의 동작을 처리할 정도로 빨랐다.)`

> 아쉽게도, 요즘 사용되는 프로세서에 비교하면 아무것도 아닌 속도다..  
  `(우리가 사용하는 전자 기기들에는 GHz 속도의 CPU가 적용되어 있다.)`
> - 1MHz(메가헤르츠) 는 초당 백만 개의 동작을 처리한다.
> - 1GHz(기가헤르츠) 는 초당 수십억 개의 동작을 처리한다.
