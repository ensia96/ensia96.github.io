...
serializers.Serializer 는 데이터의 양식이 맞는지 검증, 객체를 딕셔너리로 직렬화해줌 / 단일객체, 여러객체 모두 적용 가능
	...
	클래스 내에서 함수를 정의하여 직렬화된 정보를 사용할 수 있음
		def create(self, validated_data) => 클래스 자가상속, sereializers.Seriealizer 에서 상속받을 수 있는 속성인자인 validated_data 설정
			return 모델클래스.create(**validated_data) => 직렬화된 데이터를 kwargs 형태로 모델클래스의 create 메서드에 인자로써 주입 (db에 저장됨)
		def update(self, instance, validated_data) => 클래스 자가상속, sereializers.Seriealizer 에서 상속받을 수 있는 속성인자인 instance, validated_data 설정
			...
			instance.save() => 정보들이 수정된 상태의 데이터 객체를 .save 메서드로 저장 (db에 저장됨)
			return instance => 정보들이 할당되어 수정된 상태의 데이터객체를 리턴

	4. 위에서 create, update 에 대한 처리를 정의해두었다면, 위의 ' 3 ' 의 상태에 .save 메서드를 호출했을때, 실제로 생성과 수정에 대한 처리가 이루어짐
	5. 별도의 kwargs 를 이용하면 request 에 포함되어 있지 않은 정보를 추가적으로 주입 가능함 (사용자나 시간 등)
	6. 인스턴스를 저장하는 것이 아니라 , 그냥 사용해야 한다면 def save(self) 로 .save 메서드를 오버라이딩하면 됨 (validated_data 속성에 직접접근 가능)

	deserialize 역직렬화를 할 때, validated_data 에 접근하기 위해선 .is_valid() 메서드를 필수적으로 호출해야함.
	이 과정에서 .is_valid() 가 false 를 리턴했을때, .errors 메서드를 호출해서 key 가 필드명이고, value 가 에러내용이 담긴 리스트인 객체를 리턴받을 수 있음
	...
	.is_valid 메서드는 raise_exception 이라는 선택적 속성을 부여받아, serializers.ValidationError 처리를 할 수 있음
	...
	여러 필드에 대한 판별이 필요할경우 .validate() 라는 메서드를 시리얼라이저의 서브클래스로 등록하면 됨.
...
renderers.JSONRenderer 는 파이썬의 딕셔너리를 JSON 으로 렌더링 해줌
	...
	1. JSONRenderer() 를 호출한 뒤에 .render 메서드를 호출, 이때 인자는 시리얼라이저가 .data 메서드로 리턴한 객체를 넣음
	2. 결과는 바이너리화 된 딕셔너리형
...
parsers.JSONParser 는 filelike 상태의 정보를 파이썬의 네이티브한 데이터로 구문분석해줌
	...
	1. JSONParser() 를 호출한 뒤에 .parse 메서드를 호출, 이때 인자는 filelike 한 상태의 정보를 넣음
	2. ' 1 ' 에서 처리된 결과를 seriealizer 에 kwarg 방식을 통해 인자로 넣음 (data=대상)
	3. ' 2 ' 의 상태에서 .is_valid() 메서드를 사용한 뒤 (return 은 true), .validated_data 메서드를 사용하면 딕셔너리를 반환함
...
이제는 __proto__ (프로토링크) 대신 getPrototypeOf 메서드로 변경
...
session 객체의 save 메서드 (인자로온 콜백을 바로 실행)
...
use : 미들웨어 등록
get : get 메서드에 대해서만 사용
post : post 메서드에 대해서만 사용
...
우리는 이런 충돌을 해결하여 중복된 값을 저장하지 않도록 해시 테이블을 사용하여 주어진 MySet클래스의 add 메서드를 간단하게 구현해 보겠습니다.
...
Python 에서 파일객체를 다루는 메서드로 open 을 사용함
open 메서드의 첫번째 인자로는 대상이 되는 리소스, 두번째 인자로는 리소스에 대한 처리방식이 옴
